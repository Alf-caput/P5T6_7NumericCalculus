...
% las comprobaciones pertinentes.
    ...
    for j = 1:ny
        dfdx(1,j) = (-3*f(1,j) + 4*f(2,j) - f(3,j)) / (2*hy);
        dfdx(nx,j) = (f(nx-2,j) - 4*f(nx-1,j) + 3*f(nx,j)) / (2*hy);
    end
    ...
    % puntos en el primer y último punto respectivamente
    for i = 1:ny
        dfdy(i,1) = (-3*f(i,1) + 4*f(i,2) - f(i,3)) / (2*hx);
        dfdy(i,ny) = (f(i,ny-2) - 4*f(i,ny-1) + 3*f(i,ny)) / (2*hy);
    end
end